...
from dataloaders.dataset_visual_evidence_wrapper import VisualEvidenceDatasetWrapper
...
def parse_gpus(gpus_str: str):
    ...
    for p in s.split(","):
        ...
        if p == "":
            continue
        out.append(int(p))
    forbidden = [gpu_id for gpu_id in out if gpu_id not in (2, 3)]
    if forbidden:
        raise ValueError(
            f"Only GPU2/GPU3 are allowed in this repo. Got forbidden gpu ids: {forbidden}"
        )
    return out
...
def _get_signature_target(model: torch.nn.Module) -> torch.nn.Module:
...
def unwrap_dataset(dataset):
    ...
    while hasattr(current, "base_dataset"):
        ...
        if next_dataset is None or id(next_dataset) in visited:
            break
...
def maybe_wrap_with_visual_evidence(dataset, args, split: str):
    if not bool(getattr(args, "aux_visual_enable", 0)):
        return dataset
    if isinstance(dataset, VisualEvidenceDatasetWrapper):
        return dataset

    raw_enable = bool(getattr(args, "aux_raw_global_enable", 0) and getattr(args, "aux_raw_global_feats_path", None))
    patch_enable = bool(getattr(args, "aux_patch_enable", 0) and getattr(args, "aux_patch_root", None))
    if not raw_enable and not patch_enable:
        return dataset

    return VisualEvidenceDatasetWrapper(
        base_dataset=dataset,
        raw_global_feats_path=getattr(args, "aux_raw_global_feats_path", None),
        patch_root=getattr(args, "aux_patch_root", None),
        patch_block=int(getattr(args, "aux_patch_block", 6)),
        raw_global_enable=raw_enable,
        patch_enable=patch_enable,
        split=split,
    )


def move_aux_tensors_to_device(aux_inputs: Dict[str, Any], device: torch.device) -> Dict[str, Any]:
    ...
    moved: Dict[str, Any] = {}
    for key, value in (aux_inputs or {}).items():
        moved[key] = _move(value)
    return moved


def prepare_generation_state_if_available(
    ...
    target_model = _get_signature_target(model)
    fn = getattr(target_model, "prepare_generation_state", None)
    if not callable(fn):
        return None
    call_kwargs = _filter_supported_kwargs(
        fn,
        {
    ...
    return fn(vid_feat, vid_mask, **call_kwargs)


def expand_generation_state_if_available(
    model: torch.nn.Module,
    generation_state: Optional[Dict[str, Any]],
    batch_size: int,
) -> Optional[Dict[str, Any]]:
    if generation_state is None:
        return None
    fn = getattr(model, "expand_generation_state", None)
    if callable(fn):
        return fn(generation_state, batch_size)
...
def select_generation_state_if_available(
    model: torch.nn.Module,
    generation_state: Optional[Dict[str, Any]],
    index: int,
) -> Optional[Dict[str, Any]]:
    if generation_state is None:
        return None
    fn = getattr(model, "select_generation_state", None)
    if callable(fn):
        return fn(generation_state, index)
...
def forward_with_generation_state(
    model: torch.nn.Module,
    vid_feat: torch.Tensor,
    vid_mask: torch.Tensor,
...
def split_batch_and_aux(batch):
    if not isinstance(batch, (tuple, list)):
        raise TypeError("batch must be a tuple/list")
    batch_tuple = tuple(batch)
    known_base_lengths = {7, 18, 22}
    if len(batch_tuple) in known_base_lengths:
        return batch_tuple, {}
    if len(batch_tuple) >= 1 and isinstance(batch_tuple[-1], dict) and len(batch_tuple) - 1 in known_base_lengths:
        return batch_tuple[:-1], dict(batch_tuple[-1])
    if len(batch_tuple) >= 5 and isinstance(batch_tuple[-5], dict) and len(batch_tuple) - 5 in known_base_lengths:
        aux_inputs = dict(batch_tuple[-5])
        aux_inputs.update(
            {
                "aux_raw_global_feats": batch_tuple[-4],
                "aux_raw_global_mask": batch_tuple[-3],
                "aux_patch_feats": batch_tuple[-2],
                "aux_patch_mask": batch_tuple[-1],
            }
        )
        return batch_tuple[:-5], aux_inputs
    base_len = len(batch_tuple) - 4
    if base_len in known_base_lengths:
        return batch_tuple[:-4], {
            "aux_raw_global_feats": batch[-4],
            "aux_raw_global_mask": batch[-3],
            "aux_patch_feats": batch[-2],
            "aux_patch_mask": batch[-1],
        }
    return batch_tuple, {}
...
def resolve_run_dir(args) -> Path:
    ...
    return Path(args.out_dir) / f"{args.dataset_type}_{args.model_type}_{timestamp}"
...
def greedy_generate_batch(
    ...
    if generation_state is None:
        generation_state = prepare_generation_state_if_available(
            model,
            vid_feat,
            vid_mask,
            aux_raw_global_feats=aux_raw_global_feats,
    ...
    for step in range(max_new_tokens):
        ...
        logits = forward_with_generation_state(
            model,
            vid_feat,
            vid_mask,
            captions,
...
def beam_search_batch(
    ...
    for i in range(B):
        vf = vid_feat[i:i+1]
        vm = vid_mask[i:i+1]
        sample_generation_state = select_generation_state_if_available(model, generation_state, i)
        ...
        for step in range(max_new_tokens):
            ...
            beam_generation_state = expand_generation_state_if_available(model, sample_generation_state, m)
            logits = forward_with_generation_state(
                model,
                vf_rep,
                vm_rep,
...
def train_one_epoch(
    ...
    for it, batch in enumerate(pbar, start=1):
        batch, aux_inputs = split_batch_and_aux(batch)
        ...
        aux_inputs = move_aux_tensors_to_device(aux_inputs, device)
...
def evaluate_on_val(
    ...
    val_dataset = maybe_wrap_with_visual_evidence(val_dataset, args, split="val")
    ...
    for batch in pbar:
        ...
        generation_state = prepare_generation_state_if_available(model, vid_feat, vid_mask, **aux_inputs)
        logits_tf = forward_with_generation_state(
            model,
            vid_feat,
            vid_mask,
            caption_ids,
...
def evaluate_on_test(
    ...
    if args.dataset_type == 'msrvtt':
        ...
        raise ValueError(f"Unsupported dataset_type: {args.dataset_type}")
    test_dataset = maybe_wrap_with_visual_evidence(test_dataset, args, split="test")
    ...
    ref_dataset = unwrap_dataset(test_dataset)
    for vid in ref_dataset.video_ids:
        caps = [c for (c, _sid) in ref_dataset.captions_data.get(vid, [])]
        if len(caps) == 0:
            caps = [""]
        refs[vid] = caps
...
def main_worker(local_rank: int, args):
    ...
    if args.dataset_type == 'msrvtt':
        ...
        raise ValueError(f"Unsupported dataset_type: {args.dataset_type}")
    train_dataset = maybe_wrap_with_visual_evidence(train_dataset, args, split="train")
...
def build_parser():
    ...
    ap.add_argument("--aux_visual_enable", type=int, default=0)
    ap.add_argument("--aux_raw_global_enable", type=int, default=0)
    ap.add_argument("--aux_raw_global_feats_path", default=None)
    ap.add_argument("--aux_patch_enable", type=int, default=0)
    ap.add_argument("--aux_patch_root", default=None)
    ap.add_argument("--aux_patch_block", type=int, default=6)
    ap.add_argument("--aux_visual_raw_global_dim", type=int, default=512)
    ap.add_argument("--aux_visual_patch_dim", type=int, default=768)
    ap.add_argument("--aux_visual_prior_scale", type=float, default=0.15)
    ap.add_argument("--aux_visual_struct_scale", type=float, default=0.10)
    ap.add_argument("--aux_visual_memory_scale", type=float, default=0.10)
